...
- **What is the optimal interface between columnar compute and an agent?** The benchmark now spans enough of the spectrum to answer this directly. The candidates, from least to most abstraction:
  ...
  **Thread with @matsonj (2026-03-06):** Jacob's take is "somewhere between 2 & 3 — a context layer. dbt is just there as a well-understood primitive with a DAG built in." The key insight: dbt may be a hangover from MDS rather than the right tool for agents. The need for an advanced semantic layer has collapsed and can be replaced with context management — but the imprecise analytics primitives in dbt (data modelling theory, or lack of) are the real concern. A coding agent pointed at dbt will just produce dbt spaghetti as trained. Redesigning dbt's role for agents — less boilerplate, more ergonomic, more precise — likely lands you at Malloy at minimum, or something new entirely. The DAG runner is the least of the worries; it's the modelling language that matters.

  Jacob also notes: "prob don't need dbt but do need an intermediate layer that's not exposed to the agent to transform the data so it's ready for querying. It's more like a cache than a transformation layer though." This reframes dbt's role: not a tool the agent uses, but a pre-computation layer the agent queries through. The benchmark should test this distinction — does hiding dbt behind a query interface (agent never sees SQL models) outperform giving the agent direct dbt access?
