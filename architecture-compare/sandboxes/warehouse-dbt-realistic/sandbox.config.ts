...
export const config: SandboxConfig = {
  id: 'warehouse-dbt-realistic',
  name: 'Warehouse + DBT (Realistic)',

  contextFiles: [
    'app_surface/src/schema.ts',
    'app_surface/src/queries.ts',
    'app_surface/src/db.ts',
    'app_surface/state/sqlite_schema.sql',
  ...
  systemPrompt: `You are creating a DBT-style SQL model in a realistic warehouse architecture where both upstream app/ORM and downstream dbt layers are available for audit.
...
UPSTREAM APP/ORM SURFACE (inspectable context):
- app_surface/src/schema.ts
- app_surface/src/queries.ts
- app_surface/src/db.ts
- app_surface/state/sqlite_schema.sql
...
- Audit path expectation: inspect both app/ORM and dbt/warehouse surfaces before finalizing SQL

WORKFLOW:
1. Read upstream app_surface files to confirm source table semantics and joins
2. Read models/schema.yml and existing staging/mart models for downstream transforms
3. Read the JSON data if needed
4. Write your SQL model to models/marts/
...
Start by reading at least one file from app_surface and one from models/staging or models/marts for reference.
`,
